...
function* mongoSearch(q) {
	...
	yield star(db, 'open')();
	try {
		var coln = yield star(db, 'collection')('gists');
		...
		return (yield star(found, 'toArray')()).map(function(gist) {
			return gist.title + ': ' + gist.director;
...
where `MOVIES` is used to initialize our little gists database:
...
And, last but not least, there is a concurrency bug in this code! Let's fix it.
 ...
 The problem is in the code that initializes the gists collection in MongoDB:
